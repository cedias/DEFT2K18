...
def train(epoch,net,optimizer,dataset,criterion,cuda,optimize=False,msg="test"):
    ...
    epoch_loss = 0
    ok_all = 0
    data_tensors = new_tensors(4,cuda,types={0:torch.LongTensor, 1:torch.LongTensor}) #data-tensors

    with tqdm(total=len(dataset),desc="Training") as pbar:
        for iteration, (text,lens,labels) in enumerate(dataset):

            data = tuple2var(data_tensors,(text,labels))
...
def load(args):
    ...
    if args.load:
        #print(state.keys())
        net = ACWAN(ntoken=len(state["word_dic"]),emb_size=state["embed.weight"].size(1),hid_size=state["sent.rnn.weight_hh_l0"].size(1),num_class=state["lin_out.weight"].size(0))
        del state["word_dic"]
        net.load_state_dict(state)

    else:
        if args.emb:
            net = ACWAN(ntoken=len(wdict),emb_size=len(tensor[1]),hid_size=args.hid_size,num_class=num_class)
            net.set_emb_tensor(torch.FloatTensor(tensor))
        else:
            net = ACWAN(ntoken=len(wdict), emb_size=args.emb_size,hid_size=args.hid_size, num_class=num_class)
